...
#include UE_INLINE_GENERATED_CPP_BY_NAME(AfterAssetManager)
...
#define STARTUP_JOB_WEIGHTED(JobFunc, JobWeight)                                                                                  \
    StartupJobs.Add(FAfterAssetManagerStartupJob(                                                                                 \
        #JobFunc, [this](const FAfterAssetManagerStartupJob& StartupJob, TSharedPtr<FStreamableHandle>& LoadHandle) { JobFunc; }, \
        JobWeight))
...
#if WITH_EDITOR
void UAfterAssetManager::PreBeginPIE(bool bStartSimulate)
{
    Super::PreBeginPIE(bStartSimulate);

    {
        FScopedSlowTask SlowTask(0, NSLOCTEXT("AfterEditor", "BeginLoadingPIEData", "Loading PIE Data"));
        const bool bShowCancelButton = false;
        const bool bAllowInPIE = true;
        SlowTask.MakeDialog(bShowCancelButton, bAllowInPIE);

        const UAfterGameData& LocalGameDataCommon = GetGameData();
    ...
    TSubclassOf<UPrimaryDataAsset> DataClass, const TSoftObjectPtr<UPrimaryDataAsset>& DataClassPath, FPrimaryAssetType PrimaryAssetType)
{
    ...
    {
#if WITH_EDITOR
        FScopedSlowTask SlowTask(0, FText::Format(NSLOCTEXT("AfterEditor", "BeginLoadingGameDataTask", "Loading GameData {0}"),
...
void UAfterAssetManager::InitializeGameplayCueManager()
{
    ...
    checkf(GCM, TEXT("Failed to receive GameplayCueManager!!!"));
